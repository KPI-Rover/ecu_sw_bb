...
class ProtocolTest : public ::testing::Test {
   protected:
    ...
    void SetUp() override {
        ...
        // For motors_
        for (int i = 1; i <= 4; i++) {
            ON_CALL(GetMockRCEncoder(), read(i)).WillByDefault(::testing::Return(42));
            ON_CALL(GetMockRCEncoder(), write(i, 0)).WillByDefault(::testing::Return(0));
        }

        // Set up actual expectations for initialization
        EXPECT_CALL(getStartStopMock(), kill_existing_process(::testing::_)).WillOnce(::testing::Return(0));
        EXPECT_CALL(getStartStopMock(), enable_signal_handler()).WillOnce(::testing::Return(0));
        EXPECT_CALL(GetMockRCMotor(), init_freq(::testing::_)).WillOnce(::testing::Return(0));
...
TEST_F(ProtocolTest, GetApiVersionTest) {
    std::vector<uint8_t> request = {ProtocolHanlder::kIdGetApiVersion};
    std::vector<uint8_t> response = protocol_handler->HandleMessage(request);

    ASSERT_EQ(response.size(), 2);
    ASSERT_EQ(response[0], ProtocolHanlder::kIdGetApiVersion);
...
TEST_F(ProtocolTest, SetMotorSpeedTest) {
    ...
    EXPECT_CALL(GetMockRCMotor(), set(librc_motor_id, ::testing::_)).WillOnce(::testing::Return(0));

    std::vector<uint8_t> request = {ProtocolHanlder::kIdSetMotorSpeed, protocol_motor_id};
    request.insert(request.end(), reinterpret_cast<uint8_t*>(&rpm_network_order),
                   reinterpret_cast<uint8_t*>(&rpm_network_order) + sizeof(int32_t));

    std::vector<uint8_t> response = protocol_handler->HandleMessage(request);

    ASSERT_EQ(response.size(), 1);
    ASSERT_EQ(response[0], ProtocolHanlder::kIdSetMotorSpeed);
}
...
TEST_F(ProtocolTest, SetAllMotorsSpeedTest) {
    ...
    for (int i = 1; i <= 4; i++) {
        EXPECT_CALL(GetMockRCMotor(), set(i, ::testing::_)).WillOnce(::testing::Return(0));
    }

    std::vector<uint8_t> request = {ProtocolHanlder::kIdSetAllMotorsSpeed};
    ...
    ASSERT_EQ(response.size(), 1);
    ASSERT_EQ(response[0], ProtocolHanlder::kIdSetAllMotorsSpeed);
}
...
TEST_F(ProtocolTest, GetEncoderTest) {
    ...
    EXPECT_CALL(GetMockRCEncoder(), read(librc_motor_id)).WillOnce(::testing::Return(encoder_value));
    EXPECT_CALL(GetMockRCEncoder(), write(librc_motor_id, 0)).WillOnce(::testing::Return(0));

    std::vector<uint8_t> request = {ProtocolHanlder::kIdGetEncoder, protocol_motor_id};
    std::vector<uint8_t> response = protocol_handler->HandleMessage(request);

    ASSERT_EQ(response.size(), 5);  // 1 byte for command ID + 4 bytes for encoder value
    ASSERT_EQ(response[0], ProtocolHanlder::kIdGetEncoder);
...
TEST_F(ProtocolTest, GetAllEncodersTest) {
    ...
    std::vector<uint8_t> request = {ProtocolHanlder::kIdGetAllEncoders};
    std::vector<uint8_t> response = protocol_handler->HandleMessage(request);
...
TEST_F(ProtocolTest, MotorsStopMessageTest) {
    ...
    ASSERT_EQ(stop_message[0], ProtocolHanlder::kIdSetAllMotorsSpeed);
...
TEST_F(ProtocolTest, SetMotorSpeedErrorTest) {
    ...
    EXPECT_CALL(GetMockRCMotor(), set(librc_motor_id, ::testing::_)).WillOnce(::testing::Return(-1));  // Simulate error
