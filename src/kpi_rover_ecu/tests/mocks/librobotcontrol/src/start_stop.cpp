...
rc_state_t rc_get_state(void) {
    return StartStopMock::getInstance().get_state();
}
...
int rc_print_state(void) {
    return StartStopMock::getInstance().print_state();
}

int rc_make_pid_file(void) {
    return StartStopMock::getInstance().make_pid_file();
}

int rc_kill_existing_process(float timeout_s) {
    return StartStopMock::getInstance().kill_existing_process(timeout_s);
}

int rc_remove_pid_file(void) {
    return StartStopMock::getInstance().remove_pid_file();
}

int rc_enable_signal_handler(void) {
    return StartStopMock::getInstance().enable_signal_handler();
}

int rc_disable_signal_handler(void) {
    return StartStopMock::getInstance().disable_signal_handler();
}
...
void setupDefaultMockActions() {
    ...
    ON_CALL(mock, get_state())
        .WillByDefault(::testing::Return(current_state));
        
    ON_CALL(mock, set_state(::testing::_))
        .WillByDefault([](rc_state_t state) { current_state = state; });
        
    ON_CALL(mock, print_state())
        .WillByDefault(::testing::Return(0));
        
    ON_CALL(mock, make_pid_file())
        .WillByDefault(::testing::Return(0));
        
    ON_CALL(mock, kill_existing_process(::testing::_))
        .WillByDefault(::testing::Return(0));
        
    ON_CALL(mock, remove_pid_file())
        .WillByDefault(::testing::Return(0));
        
    ON_CALL(mock, enable_signal_handler())
        .WillByDefault(::testing::Return(0));
        
    ON_CALL(mock, disable_signal_handler())
        .WillByDefault(::testing::Return(0));
}
