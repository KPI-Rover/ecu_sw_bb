...
TEST_F(MotorTest, MotorGoCallsRcMotorSet) {
    ...
    EXPECT_CALL(GetMockRCMotor(), set(0, ::testing::DoubleEq(1)))
        .WillOnce(::testing::Return(0));
    
    motor = new Motor(0, false);
    int result = motor->MotorGo(26500);
...
TEST_F(MotorTest, MotorGoMinRPM) {
    EXPECT_CALL(GetMockRCMotor(), set(0, ::testing::_))
        .WillOnce(::testing::Return(0));
    
    motor = new Motor(0, false);
    int result = motor->MotorGo(MIN_RPM);
...
TEST_F(MotorTest, MotorGoInverted) {
    ...
    EXPECT_CALL(GetMockRCMotor(), set(0, ::testing::Lt(0)))
        .WillOnce(::testing::Return(0));
    
    motor = new Motor(0, true);
    int result = motor->MotorGo(MAX_RPM);
...
TEST_F(MotorTest, MotorGoBelowMinRPM) {
    ...
    motor = new Motor(0, false);
    int result = motor->MotorGo(MIN_RPM - 1);
...
TEST_F(MotorTest, MotorGoAboveMaxRPM) {
    ...
    motor = new Motor(0, false);
    int result = motor->MotorGo(MAX_RPM + 1);
...
TEST_F(MotorTest, MotorStop) {
    ...
    EXPECT_CALL(GetMockRCMotor(), brake(0))
        .WillOnce(::testing::Return(0));
...
TEST_F(MotorTest, GetEncoderCounter) {
    EXPECT_CALL(GetMockRCEncoder(), read(0))
