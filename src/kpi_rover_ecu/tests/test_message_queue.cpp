...
TEST_F(MessageQueueTest, PopImmediateTest) {
    vector<uint8_t> message = {5, 6, 7, 8};
    queue.Push(message);
...
TEST_F(MessageQueueTest, ClearTest) {
    vector<uint8_t> message1 = {1, 2, 3};
    vector<uint8_t> message2 = {4, 5, 6};
...
TEST_F(MessageQueueTest, FIFOTest) {
    vector<uint8_t> message1 = {1, 2, 3};
    vector<uint8_t> message2 = {4, 5, 6};
    vector<uint8_t> message3 = {7, 8, 9};
    
    queue.Push(message1);
    queue.Push(message2);
    queue.Push(message3);
    
    vector<uint8_t> received1, received2, received3;
...
TEST_F(MessageQueueTest, ThreadedTest) {
    ...
    std::thread producer([this, numMessages]() {
        for (int i = 0; i < numMessages; i++) {
            vector<uint8_t> message = {static_cast<uint8_t>(i & 0xFF)};
            queue.Push(message);
