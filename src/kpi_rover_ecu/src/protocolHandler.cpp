...
ProtocolHanlder::ProtocolHanlder(MotorController* motorDriver) { mainController = motorDriver; }
...
vector<uint8_t> ProtocolHanlder::handleSetMotorSpeed(vector<uint8_t> message) {
    ...
    if (mainController->setMotorRPM(static_cast<int>(motor_id), static_cast<int>(motor_rpm)) != 0) {
